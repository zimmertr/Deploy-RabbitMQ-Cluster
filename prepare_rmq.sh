...
### first, and will provision the first node and initialize the cluster. Afterwards you can run the second###
### script to configure that node, and join to this cluster. This script will do the following:           ###
###                                                                                                       ###
...
###  - Upgrade packages and install rabbitmq                                                              ###
###  - Modify the earling encryption cookie                                                               ###
###  - Disable the firewall                                                                               ###
###  - Set a higher value for linux ulimit                                                                ###
###  - Initialize the RMQ node                                                                            ###
###                                                                                                       ###
### YOU MUST WAIT until this script has finished before continuing onto the next.                         ###
#############################################################################################################
#############################################################################################################

#Create LVM partition on the Disk and format it as ext4.
...
echo 'address >IPADDR<' | sudo tee --append /etc/network/interfaces.d/50-cloud-init.cfg > /dev/null
